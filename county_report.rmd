...
ggplot(visitagg, aes(x = plot_qtr, 
    ...
    expand_limits(y = 0) +
    scale_x_discrete(breaks = qtr.seq[4:12]) +
...
act_comp <- ddply(subset(latest_plans_co, treat_plan_type %in% "Active"),
                 ...
                 per_complete = calc_percent(n_complete, n)
...
act_comp$variable <- "Cases in County"
...
# Plot it
ggplot(act_plot, aes(x = plot_qtr, y = as.numeric(per_complete), 
...
ltbi_comp <- ddply(subset(plans_trim, 
           ...
           n_ongoing = sum(completion_status %in% "Ongoing"),
           per_complete = calc_percent(n_complete, n),
           per_incomplete = calc_percent(n_incomplete, n),
           per_ongoing = calc_percent(n_ongoing, n)
)
